...
bot.on("error", (err) => {
  //most telegram errors should get caught here
  console.log(
    "Telegram Error " +
  ...
  return axios
    ...
    .catch((err) => {
      ...
      try {
        ...
        console.log(
          "Error from error sender report.error did not work: ",
          JSON.stringify(e)
        );
      }
...
process.on("unhandledRejection", function (reason, p) {
  ...
  console.log(
    "Possibly Unhandled Rejection at - Promise reason: ",
    reason.description,
    " - p: ",
    p
  ); //  ", " p,
  try {
    ...
    console.log(
      "Error report.error on unhandledRejection: ",
      JSON.stringify(e)
    );
  }
...
bot.on("inlineQuery", (msg) => {
  userCache(msg);
  let lang =
    dataOnUser[msg.from.id]["lang"] !== (undefined && null)
      ? dataOnUser[msg.from.id]["lang"]
